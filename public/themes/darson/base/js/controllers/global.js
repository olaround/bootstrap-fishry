function GlobalCtrl($scope,$location,$rootScope,ngMap){
...
function PageCtrl($scope,$location,AzureMobileClient,$rootScope,$routeParams,$cookies) {	
 ...
 // console.log('qwerty');
    ...
    $rootScope.sendaskDoctorEmail = function() {
        ...
        $rootScope.toBCC = '';
        $rootScope.subjectEmail = 'Contact us via Darson';
        $rootScope.emailBody = '<h1>Contact Details</h1><table style="width:900px"><tr><td>Name</td><td>Email</td><td>Phone</td><td>Country</td><td>Department</td><td>Message</td></tr><tr><td>' + $rootScope.askDoctor.FirstName + '</td><td>'+ $rootScope.askDoctor.Email + '</td><td>' + $rootScope.askDoctor.Phone + '</td><td>' + $rootScope.askDoctor.Country + '</td><td>' + $rootScope.askDoctor.Department + '</td><td>' + $rootScope.askDoctor.height + '</td></tr></table>';
        var dataEmail = {
        ...
        $location.path('/page/contactthanks');
    }
}
// JavaScript Document
