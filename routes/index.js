...
var renderApp = function(req, res){
	  ...
	  var url = req.protocol + '://' + req.get('host') + req.originalUrl;
		  var BaseUrl = req.get('host').split('.');
					  ...
					  if(req.get('host') == 'fishry.com' || req.get('host') == 'fishry.com:3000' || req.get('host') == 'www.fishry.com' || req.get('host') == 'www.fishry.com:3000'){
...
var loadAppEjs = function(req,res){
   ...
   if(req.route.path == '/collections/:id' && req.route.params.id != 'all'){
	...
	if(target){
		var requestUrl = 'http://ae-commerce.azure-mobile.net/api/fishrysdk?store_id='+app.locals.storeName+'&'+target+ '='+targetParam;
	}else{
		var requestUrl = 'http://ae-commerce.azure-mobile.net/api/fishrysdk?store_id='+app.locals.storeName;
	}
	...
	////console.log(requestUrl);
	  ...
	  if (!error && response.statusCode == 200) {
		 // //console.log('http://ae-commerce.azure-mobile.net/api/fishrysdk?store_id='+app.locals.storeName);
				var dataBody = JSON.parse(body);
				...
				if(dataBody.settings_general[0]){				
					...
					app.locals.EjsFilePath = 'themes/'+dataBody.settings_general[0].hostName+'/active/views/';
					app.locals.ThemeFolder = 'public/themes/'+dataBody.settings_general[0].hostName+'/';
					app.locals.ThemeFolderPath =  'public/themes/'+dataBody.settings_general[0].hostName+'/active/views/';
					app.locals.ThemeFolderPathBase =  '/themes/'+dataBody.settings_general[0].hostName+'/active/base/';	
					app.locals.ThemeFolderPathBaseAdmin =  '/themes/'+dataBody.settings_general[0].hostName+'/active/base/partials/';	
					app.locals.EjsFilePath = 'themes/'+dataBody.settings_general[0].hostName+'/active/views/';
		...
		var imgOg = 'http://fishry.com/frontend/img/theme/og-fishry.png';
		if(req.route.path == '/'){	
				var data = {
